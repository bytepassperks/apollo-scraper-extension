...
async function init() {
  ...
  chrome.runtime.onMessage.addListener(message => {
    if (message.type === "run-status") $("progress").textContent = `Pages fetched: ${message.status.pages || 0} · Records collected: ${message.status.records || 0}`;
    if (message.type === "run-complete") { $("download").disabled = false; $("progress").textContent = `Complete · ${message.records.length} records`; }
  });
}
...
$("download").onclick = async () => { const value = settings(); await chrome.runtime.sendMessage({ type: "download", format: value.format, fields: value.fields, allFields: value.allFields }); };
