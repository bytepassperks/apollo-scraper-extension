...
export const COMPANY_FIELDS = [
  ["Account ID", "account_id"], ["Company Name", "name"], ["LinkedIn URL", "linkedin_url"],
  ["Website URL", "website_url"], ["Twitter URL", "twitter_url"], ["Facebook URL", "facebook_url"],
  ["Primary Domain", "primary_domain"], ["Industry", "industry"], ["Founded Year", "founded_year"],
  ["Employee Count", "employee_count"], ["Phone Number", "phone_number"],
...
const companyAliases = {
  account_id: ["id", "account_id", "accountId", "organization_id", "organizationId"],
  name: ["name", "organization_name", "organizationName"],
  linkedin_url: ["linkedin_url", "linkedinUrl", "linkedin"],
  website_url: ["website_url", "websiteUrl", "domain"],
  twitter_url: ["twitter_url", "twitterUrl", "twitter"],
  facebook_url: ["facebook_url", "facebookUrl", "facebook"],
  primary_domain: ["primary_domain", "primaryDomain", "domain"],
  industry: ["industry", "industry_name"],
  founded_year: ["founded_year", "foundedYear"],
  employee_count: ["employee_count", "employeeCount", "estimated_num_employees"],
...
export function flattenCompanyRecord(record) {
  const result = {};
  for (const [, key] of COMPANY_FIELDS) {
    const value = (companyAliases[key] || [key]).map(candidate => record?.[candidate]).find(candidate => candidate !== undefined);
    result[key] = printable(value);
  }
  return result;
}
...
export function fieldDefinitions(entityKind = "people") {
  return entityKind === "companies" ? COMPANY_FIELDS : DEFAULT_FIELDS.concat([["Current Employer", "current_employer"], ["Current Role", "current_role"]]);
}
