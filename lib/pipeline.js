...
export async function collectPages({ requestBody, fetchPage, maxRecords = 1000, maxPages = 100, delayMs = 1000, jitter = 250, onProgress = () => {} }) {
  ...
  for (let pages = 0; pages < maxPages && records.length < maxRecords; pages++, page++) {
    ...
    if (!response || response.status < 200 || response.status >= 300) throw new Error(`Apollo request failed (HTTP ${response?.status || "unknown"}).`);
