...
export function replayHeaders(headers = {}) {
  const oneShotHeaders = new Set(["x-cf-turnstile-response", "x-cf-widget-type"]);
  return Object.fromEntries(Object.entries(headers).filter(([key]) => !oneShotHeaders.has(key.toLowerCase())));
}

export function refreshReplayBody(body, now = Date.now()) {
  if (!body || typeof body !== "object" || Array.isArray(body) || !Object.prototype.hasOwnProperty.call(body, "cacheKey")) return body;
  return { ...body, cacheKey: now };
}
